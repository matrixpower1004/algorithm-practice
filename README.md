...
- `int floor = (N - 1) % H + 1;`
- N번째 손님의 호수 : H로 나눈 몫. 테스트 케이스가 1~10이라고 했을 때 1~9 까지는 몫이 0이 나오고 10만 몫이 1이 나온다. 층수를 구할 때와 마찬가지로 `N-1`을 먼저 해준 값을 H로 나누고 그 결과에 1을 더해준다.
- 결론적으로 1~H => 1 / H+1 ~ 2H => 2로 변환하는 작업을 해줘야 한다.
